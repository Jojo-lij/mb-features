...
**仓库**：`github.com/Jojo-lij/mb-features`，本地路径 `APP功能说明/`

**上游关联**：已建立 `main` → `origin/main`，直接 `git push` 即可
...
**目录结构红线**：禁止提交 `APP功能说明/` 外层文件夹，GitHub Pages 根目录必须是 `index.html`
...
4. **多张图规则** — 用户说"X张图" = 该功能有 X 张截图，汇总描述，不拆功能点
...
- 展示位置、布局方式、内容类型、交互说明、状态变化、动画过渡、筛选/搜索、加载机制、空态处理、暗黑模式适配

**颗粒度原则**：每个独立维度单独成点，不合并。例如：
- ❌ `4.横向滑动：卡片支持横向滑动，Logo放大，图片切换`（三条合并）
- ✅ `4.中央品牌高亮：滑动时位于屏幕中央的品牌Logo放大显示`
- ✅ `5.背景图切换：滑动过程中卡片背景图随中央品牌切换而渐变过渡`
- ✅ `6.无限循环：品牌卡片支持无限循环滑动`
